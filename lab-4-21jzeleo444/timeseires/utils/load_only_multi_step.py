...
"""
import numpy as np
def load_only_multi_step(dataset, time_steps, target_col ,target_len):
    '''
    ...
    for i in range(len(dataset)):
        ...
        if (end_of_x+target_len)> len(dataset)-1:
            break
        seq_x = dataset[i:end_of_x, target_col ]
        X.append(seq_x)
        target_column = dataset[:,target_col]
        seq_y=target_column[end_of_x:end_of_x+target_len]    
        y.append(seq_y)
    return np.array(X).reshape(dataset.shape[0]-(time_steps+target_len),time_steps, -1), np.array(y) #.reshape(-1,1)